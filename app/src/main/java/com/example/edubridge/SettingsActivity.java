...
public class SettingsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 3. Language Switching (Merged from Local)
        ...
        // 4. Accessibility Mode (TTS) Logic
        ...
        if (user != null) {
            ...
            db.collection("users").document(uid).get().addOnSuccessListener(snapshot -> {
                if (snapshot.exists()) {
                    ...
                    if (switchCourseUpdates != null) switchCourseUpdates.setChecked(prefCourse == null || prefCourse);
                    if (switchReminders != null) switchReminders.setChecked(prefReminder == null || prefReminder);
                    if (switchAchievements != null) switchAchievements.setChecked(prefAchieve == null || prefAchieve);
                    if (switchAnnouncements != null) switchAnnouncements.setChecked(prefAnnounce == null || prefAnnounce);
                }
            });

            CompoundButton.OnCheckedChangeListener prefListener = (buttonView, isChecked) -> {
                java.util.Map<String, Object> notifPrefs = new java.util.HashMap<>();
                if (switchCourseUpdates != null) notifPrefs.put("prefCourseUpdates", switchCourseUpdates.isChecked());
                if (switchReminders != null) notifPrefs.put("prefReminders", switchReminders.isChecked());
                if (switchAchievements != null) notifPrefs.put("prefAchievements", switchAchievements.isChecked());
                if (switchAnnouncements != null) notifPrefs.put("prefAnnouncements", switchAnnouncements.isChecked());

                db.collection("users").document(uid)
                        .set(notifPrefs, com.google.firebase.firestore.SetOptions.merge());
            };

            if (switchCourseUpdates != null) switchCourseUpdates.setOnCheckedChangeListener(prefListener);
            if (switchReminders != null) switchReminders.setOnCheckedChangeListener(prefListener);
            if (switchAchievements != null) switchAchievements.setOnCheckedChangeListener(prefListener);
            if (switchAnnouncements != null) switchAnnouncements.setOnCheckedChangeListener(prefListener);
